...
export function SmoothCursor({
  ...
  const rotation = useSpring(disableRotation ? RESTING_ROTATION : 0, {
    ...springConfig,
    damping: 60,
    stiffness: 300,
  });
  ...
  useEffect(() => {
    if (!smooth) {
      const handleMouseMove = (e: MouseEvent) => {
        ...
        const nextTarget: HoverTarget = isInput
          ? 'input'
          : isButton
            ? 'button'
            : null;
  ...
  const cursorStyle: React.CSSProperties & Record<string, string> = {
    '--cursor-fill': hoveredTarget === 'button' ? 'var(--primary)' : '#1f1f1f',
    '--cursor-stroke': hoveredTarget === 'input' ? 'var(--primary)' : '#ffffff',
